...
//}
#pragma omp parallel default(none) shared(totalCorrect, noCset, nonRepReads, canCorrect, cannotCorrect, totalChanged,totalUnchanged,moreThanOneCset, cout,kmerSize,pool,ioh,trie,pools2bacs,reads,numReads,out,numSearch,numHit) private(numPools,pools,numBacs,bacs,crtKmer,pos,kmerPos,orient,kmers,kmersInHash,orientation,info) 
{
ostringstream os;
...
while (true) {
	...
	//if ((crt->bacs.size()>0)&&(crt->disagree)&&(crt->end - crt->begin + 1 <=3)) 
	//	continue;
