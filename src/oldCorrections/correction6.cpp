...
bool IOF::correct(int pool, char* read, int direction, int pos, int kmerPos, struct cset& crt, const struct cset& neighbor, const mybitsetx& e, mybitsetx c, unsigned short orient, HashTable<mysig>& pools2bacs, unsigned int& numSearch, unsigned int& numHit) {
...
if (numPools>=LOW&&numPools<=HIGH) {
	...
	if (!found) {//check neighbor cset too
		if (neighbor.end!=-1) {//neighbor can be either prev or next cset
			for (i=0;i<numBacs;i++) {
	...
	if (found||(crt.bacs.size()==0&&neighbor.bacs.size()==0)) {
		correct=true;
		for (i=0;i<numBacs;i++) 
...
if (correct) {
	...
	cout << "[correct]crtCset begin " << crt.begin << " crtCset end " << crt.end << endl; 
#endif
	return true;
}//if (correct)
...
while (crtCset<csets.size()) {
	...
	int direction=0;
	struct cset neighbor;
	neighbor.begin=neighbor.end=-1;
