...
while (true) {
	...
	if ((crt->end - crt->begin + 1 <3)&&(crt->begin - 1 > prevEnd)&&(crt->end + 1 < nextBegin))
	...
	for (auto it1=conflictingCsets.begin();it1!=conflictingCsets.end();it1++) {
		...
		if (found) {
			it1->insert(crt->bacs.begin(),crt->bacs.end());
			break;
		}	
	}//end for
...
//cout << endl;
		...
		//		if ((it2->bacs.size()==0)||(it1->find(*(it2->bacs.begin()))!=it1->end()))
