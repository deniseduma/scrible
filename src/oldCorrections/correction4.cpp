...
void IOF::checkIntermediateKmers(int pool, char* read, const vector<int>& posChanged, int direction, int begin, int end, struct cset* crt, struct cset* neighbor, const vector<mybitsetx>& kmers, const vector<unsigned short>& orientation) {
...
while (true) {
	...
	if (sh.searchCopy(g)) {//search kmer in hashtable
		...
		if (numPools>=4&&numPools<=MAXREP) {
			...
			for (i=0;i<numBacs;i++) {//check crt cset
...
bool IOF::correct(int pool, char* read, int direction, int pos, int kmerPos, struct cset* crt, struct cset* neighbor, const mybitsetx& e, mybitsetx c, unsigned short orient) {
...
for (int r=0;r<numReads;r++) { 
	...
	for (auto it2=csets.rbegin();it2!=csets.rend();it2++) {
		...
		found=false;
		for (i=0;i<numBacs;i++) {//does this kmer belong to crt cset?
			if (crt.bacs.find(bacs[i])!=crt.bacs.end()) {
				found=true;
				break;
			}
...
while (crtCset<csets.size()) {
	...
	if (updated) {
		...
		cout << "Trying " << c;
#endif		
		corrected=correct(pool, read, direction, pos, kmerPos, &csets[crtCset], neighbor, e, c, orient);
		if (corrected) {
			if (direction==0)
				{begin=bCset - 1; end=prevECset;}
			else 
				{begin=eCset + 1; end=nextBCset;}
			checkIntermediateKmers(pool, read, posChanged, direction, begin, end, &csets[crtCset], neighbor, kmers, orientation);
			continue;
		}
	}	
