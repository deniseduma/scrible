...
void findKmerToCorrect(char* read, const vector<cset>& csets, const vector<mybitsetx>& kmers, const vector<unsigned short>& orientation, unsigned short& crtCset, int& prevECset, int& nextBCset, unsigned short& direction, int& crtKmer, unsigned short& orient, int& pos, int& kmerPos, mybitsetx& c ) {

	unsigned int num=strlen(read);
	short oldCset=crtCset - 1;
	unsigned short nextCset=crtCset + 1;
...
cout << "[dfs]Trying " << c;
#endif	
	csetToDelete=dfs(read, original, direction, crtKmer, pos, kmerPos, c, kmers, kmersInHash, orientation, csets, crtCset, prevECset, nextBCset, depth, posChanged, minCorrections, numCorrections, numSearch, numHit);
...
#endif		
#ifdef DEBUG
cout << endl << "[correctReads]CSETS" << endl;
for (auto crt=csets.begin();crt!=csets.end();crt++) {
	cout << "[correctReads]cset begin " << crt->begin << ", end " << crt->end << " bacs ";
	for (auto it=crt->bacs.begin();it!=crt->bacs.end();it++)
		cout << (*it) << ",";
	cout << endl;
}
cout << endl;
...
cout << "[correctReads]Trying " << c;
			...
			csetToDelete=dfs(read, original, direction, crtKmer, pos, kmerPos, c, kmers, kmersInHash, orientation, csets, crtCset, prevECset, nextBCset, depth, posChanged, /*correction,*/ minCorrections, numCorrections, numSearch, numHit);
		...
		if (!correctable) {
#if defined DEBUG
cout << "\t[correctReads]csetToDelete " << csetToDelete << endl;
#endif
			//remove conflicting c-set
			if (!(csetToDelete>=0&&csetToDelete<csets.size())) 
			{cout << "csetToDelete " << csetToDelete << endl;exit(1);}
			crtCset=(csetToDelete>0) ? (csetToDelete - 1) : 0;
			csets.erase(csets.begin() + csetToDelete);
			if (csets.size()==0)
				break;
			findKmerToCorrect(read, csets, kmers, orientation, crtCset, prevECset, nextBCset, direction, crtKmer, orient, pos, kmerPos, c);
			toTry[0]=c.getBase(kmerPos);
