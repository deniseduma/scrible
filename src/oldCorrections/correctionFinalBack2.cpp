...
cout << "[dfs]Trying " << c;
#endif	
	corrected2=dfs(read, original, direction, crtKmer, pos, kmerPos, c, kmers, kmersInHash, orientation, csets, crtCset, prevECset, nextBCset, depth, posChanged, minCorrections, numCorrections, numSearch, numHit);
...
if (!correctable)
//	csets.erase(csets.begin() + crtCset - 1);
	assert(csets.begin() + crtCset - 1 < csets.end());
...
cout << "[correctReads]Trying " << c;
			...
			corrected=dfs(read, original, direction, crtKmer, pos, kmerPos, c, kmers, kmersInHash, orientation, csets, crtCset, prevECset, nextBCset, depth, posChanged, /*correction,*/ minCorrections, numCorrections, numSearch, numHit);
