...
cout << "[dfs]Trying " << c;
#endif	
	dfs(read, original, direction, crtKmer, pos, kmerPos, c, kmers, kmersInHash, orientation, csets, crtCset, prevECset, nextBCset, depth, posChanged, /*correction,*/ minCorrections, numCorrections, numSearch, numHit);
...
unsigned int goodCorrections=0;
unsigned int badCorrections=0;
unsigned int totalChanged=0;
unsigned int totalUnchanged1=0;
unsigned int totalUnchanged2=0;

//map<unsigned int, ostringstream*> outs;
//for (unsigned int i=0;i<cores;i++) {
//	outs[i]=new ostringstream();
//}
#pragma omp parallel default(none) shared(numCorrect, nonRepReads, goodCorrections, badCorrections, totalChanged,totalUnchanged1,totalUnchanged2,cout,kmerSize,pool,ioh,trie,pools2bacs,reads,numReads,out,numSearch,numHit) private(numPools,pools,numBacs,bacs,crtKmer,pos,kmerPos,orient,kmers,kmersInHash,orientation,info) 
{
ostringstream os;
...
while (true) {
	...
	//if ((crt->end - crt->begin + 1 <3)&&(crt->begin>prev->end + 1)&&(crt->end + 1 < nextBegin))
	//if ((crt->end - crt->begin + 1 <3))
	//	continue;
...
cout << "[correctReads]Number of reads which are correct " << numCorrect << endl;
cout << "[correctReads]Number of reads changed " << totalChanged << endl;
cout << "[correctReads]Number of reads unchanged 1 " << totalUnchanged1 << endl;
cout << "[correctReads]Number of reads unchanged 2 " << totalUnchanged2 << endl;
cout << "[correctReads]Number of good corrections " << goodCorrections << endl;
cout << "[correctReads]Number of bad corrections " << badCorrections << endl;
cout << "[correctReads]Number of reads which are non-repetitive " << nonRepReads << endl;
