...
bool correct(char* read, unsigned short direction, int crtKmer, int pos, int kmerPos, mybitsetxbac c, vector<mybitsetxbac>& kmersInHash, const vector<unsigned short>& orientation, vector<cset>& csets, unsigned short crtCset, unsigned int& numSearch, unsigned int& numHit) {

bool correct=false;
//mybitsetxbac e=kmers[crtKmer];
//mybitsetxbac e=kmersInHash[crtKmer];
...
if (correct) {
	...
	if (orient)
		...
		newBase=mybitset::rev(o.getBase(kmerPos)); 
...
void dfs(char* read, char* original, unsigned short direction, int crtKmer, int pos, int kmerPos, mybitsetxbac c, const vector<mybitsetxbac>& kmers, vector<mybitsetxbac>& kmersInHash, const vector<unsigned short>& orientation, vector<cset> csets, unsigned short crtCset, int prevECset, int nextBCset, unsigned short depth, vector<unsigned short> posChanged, string& correction, unsigned short& minCorrections, unsigned int& numSearch, unsigned int& numHit) {
...
cout << "[dfs]Trying " << c;
#endif	
	dfs(read, original, direction, crtKmer, pos, kmerPos, c, kmers, kmersInHash, orientation, csets, crtCset, prevECset, nextBCset, depth, posChanged, correction, minCorrections, numSearch, numHit);
...
vector<mybitsetxbac> kmers;
vector<mybitsetxbac> kmersInHash;
...
//}
#pragma omp parallel default(none) shared(kmerSize,pool,ioh,/*trie,pools2bacs,*/reads,numReads,out,numSearch,numHit) private(numPools,pools,numBacs,bacs,crtKmer,pos,kmerPos,orient,kmers,kmersInHash,orientation/*,minCorrections,correction*/) 
{
ostringstream os;
...
//#endif
	...
	if (minCorrections==100) {

#if defined DEBUG
cout << "[correctReads]" << header << ":" << 0 << endl << endl;
#endif
		os << header << ":" << 0 << endl << original << endl;
		continue;
	}//end if
...
#if defined DEBUG
cout << "[correctReads]" << header << ":" << 1 << endl << correction << endl << endl;
#endif
		os << header << ":" << 1 << endl << correction << endl;
...
ioh=new IOF<mybitsetxbac>(argv[1], argv[2], argv[3], kmerSize, files);
ioh->ReadB();
//ioh.CheckCorrect();
