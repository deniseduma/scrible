...
#pragma omp parallel default(none) shared(totalCorrect, noCset, canCorrect, cannotCorrect, totalChanged,totalUnchanged,cout,kmerSize,pool,ioh,trie,pools2bacs,reads,numReads,out,numSearch,numHit) 
{
...
for (unsigned int r=0;r<numReads;r++) { 
	...
	if (crtCset.begin==-1)
		newBacs.insert(bacs, bacs + numBacs);
...
#endif
		os << header << endl << read << endl;
		if (it1->first.size()==0||it1->first.size()>3) {
			os << "-1 -1 -1" << endl;
			os << 0 << endl; 
		} else {
